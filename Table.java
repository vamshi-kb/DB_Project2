       ...
       implements Serializable
{
    ...
    // Using Linear hashing for indexing
...
//      case BPTREE_MAP  -> new BpTreeMap <> (KeyType.class, Comparable [].class);
    ...
    ***********************************************************************************
    ...
    public Table project (String attributes)
    {
        ...
        {
            if(index.containsKey(attrs[i]))//If attribute is not found col will return -1 which is less than zero
            {
                System.out.println("Column "+attrs[i]+" is not there");
                return null;
            }
        }

        for(var tup:tuples)
            rows.add(extract(tup,attrs));//adding the specified column's values of each tuple to rows
    ...
    public Table join (String attributes1, String attributes2, Table table2)
    {
        ...
        for (Comparable [] tup1 : this.tuples) {
            for (Comparable [] tup2 : table2.tuples) {
                int count = 0;
                //finding if there is a complete match between t_attrs and u_attrs
                for (int i = 0; i < colPosAttr1.length; i++ ) {
                    if (tup1 [ (int)colPosAttr1[i]].equals(tup2 [(int)colPosAttr2[i]])) {
                        count++;
                    }
                    //if the match found add the tup1,tup2 to the rows as a single tuple
                    if (i == colPosAttr1.length-1 && count == colPosAttr1.length ) {
     ...
     * @return  a table with tuples satisfying the equality predicate
     */
    public Table i_join (String attributes1, String attributes2, Table table2) {return null;} // i_join
    ...
    public Table join (Table table2)
    {
        ...
        //first find the columns that are in table2 but not in table1

        String[] not_common_col_t2=new String[table2.attribute.length];
        ...
        for(int i = 0 ; i < table2.attribute.length ; i++) {
            ...
            for (int j = 0; j < this.attribute.length; j++) {
                if (table2.attribute[i].equalsIgnoreCase(this.attribute[j])) {
                    flag = 1;
                }
            }
            if(flag==0)
            {
                not_common_col_t2[count_1]=table2.attribute[i];
                count_1++;
            }
        }
        String str=String.join(" ",not_common_col_t2);
        //Adding concatinated tuples from both tables
        //tuples with equal values in all common attributes are only considered
        //if there are same columns in tabbl1 and table 2 add only table1 tuple to the rows
        //else add concatinated result of both table1 and table2 to the rows
            ...
            for(var tup2:table2.tuples) {
                ...
                if(flag==1) {
                    if(count_1==0){
                        ...
                        rows.add(ArrayUtil.concat(tup1,extract(tup2,not_common_col_t2)));
                }
            }
        }

        //if there is any column from table2 that is not in table1 contact attributes and not_common_col_t2
        if(count_1>0)
            return new Table (name + count++, ArrayUtil.concat (attribute, not_common_col_t2),
